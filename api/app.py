...
# Monkey patch BatchNormalization.from_config to ignore unsupported Keras 2 kwargs
original_bn_from_config = keras.layers.BatchNormalization.from_config
@classmethod
def patched_bn_from_config(cls, config):
    for key in ['renorm', 'renorm_clipping', 'renorm_momentum', 'fused']:
        config.pop(key, None)
    return original_bn_from_config(config)

keras.layers.BatchNormalization.from_config = patched_bn_from_config
